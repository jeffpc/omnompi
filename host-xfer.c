...
static void __mem_clear(uint32_t addr, uint32_t len)
{
	...
	fullwrite(out, &cmd, sizeof(cmd));
	fullwrite(out, &xfer, sizeof(xfer));
...
static void __mem_write(uint8_t *buf, uint32_t addr, uint32_t len)
{
	...
	fullwrite(out, &cmd, sizeof(cmd));
	fullwrite(out, &xfer, sizeof(xfer));
	fullwrite(out, compress ? tmp : buf, compress ? clen : len);
...
static void done(uint32_t pc, int autoboot)
{
	enum cmd cmd;
	...
	cmd = htonl(CMD_DONE);
	xfer.pc = htonl(pc);
	xfer.autoboot = htonl(autoboot ? 1 : 0);
